...
import { updateRidingReport, deleteRidingReport, type RidingActionState } from "./actions";
import type { HorseOption } from "./riding-form";
...
/** 騎乗報告の履歴1行 (編集はインライン展開・削除は confirm 付き) */
export default function ReportRow({ report, horses }: { report: ReportRowData; horses: HorseOption[] }) {
  const [editing, setEditing] = useState(false);
  const [updateState, updateAction, updating] = useActionState<RidingActionState, FormData>(updateRidingReport, {});
  const [deleteState, deleteAction, deleting] = useActionState<RidingActionState, FormData>(deleteRidingReport, {});
  ...
  return (
    <>
      <tr className={report.fell_off ? "bg-red-50 hover:bg-red-100" : "hover:bg-gray-50"}>
        <Td className="whitespace-nowrap text-gray-700">{fmtDate(report.report_date)}</Td>
        <Td className="whitespace-nowrap font-medium text-gray-800">{report.horse_name ?? "—"}</Td>
        ...
        <Td className="max-w-[14rem]">
          <p className="whitespace-pre-wrap text-gray-600">{report.horse_condition ?? "—"}</p>
          {report.horse_mood && (
          ...
          <div className="flex items-center gap-2">
            ...
            <form
              ...
              onSubmit={(e) => {
                if (
                  !window.confirm(
                    `${fmtDate(report.report_date)} の騎乗報告(${report.horse_name ?? "馬"})を削除します。この操作は取り消せません。よろしいですか?`
                  )
                ) {
                  e.preventDefault();
                }
          ...
          <td colSpan={COL_COUNT} className="border-t border-gray-200 bg-gray-50 px-4 py-4">
            <form action={updateAction} className="space-y-4">
              ...
              <Field label="騎乗内容" required>
                <textarea name="content" required rows={4} defaultValue={report.content} className={inputCls} />
              </Field>
              ...
              <p className="text-xs text-gray-500">
                ※ 編集・削除の内容はリタッチ馬の月次AI要約には自動反映されません。反映が必要な場合は、学校職員が「リタッチ馬
