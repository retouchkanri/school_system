...
/** クライアント側の開発判定 (本番ビルドでは false) */
const DEV_SKIP_PAYMENT = process.env.NODE_ENV === "development";
  ...
  return (
    <form action={formAction} className="mt-3 border-t border-gray-100 pt-3">
      ...
      {DEV_SKIP_PAYMENT ? (
        <>
          ...
          <p className="mb-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            開発モード: 決済をスキップし、入金済み・参加済みとして体験アンケートへ進みます
          </p>
        </>
      ) : (
        <>
      ...
      <button type="submit" disabled={pending} className={`${btnPrimary} mt-3 w-full`}>
        {pending ? "送信中…" : DEV_SKIP_PAYMENT ? "予約する (決済スキップ)" : "このイベントを予約する"}
      </button>
    </form>
  );
}
