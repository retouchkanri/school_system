...
export const BANK_INFO = "千葉銀行 東金支店 普通 1234567 (口座名義: 学)トウカントウバジガクイン)";
  ...
  if (state.ok) {
    return (
      <div className="rounded-lg bg-emerald-50 p-4">
        <p className="text-sm font-bold text-emerald-800">✓ ご予約を受け付けました</p>
        {state.bank ? (
          <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
            <p className="font-bold">お振込のご案内</p>
            <p className="mt-1">
              下記口座へ参加費のお振込をお願いいたします。入金確認後、予約が確定します。
            </p>
          ...
          <p className="mt-1 text-sm text-emerald-700">クレジットカード決済が完了しました。当日お待ちしております。</p>
        )}
      </div>
    );
  }
