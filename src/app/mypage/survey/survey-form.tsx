...
function buildPages(questions: SurveyQuestion[]) {
  ...
  for (const q of questions) {
    if (q.section) {
      ...
      current = { title: "アンケート", questions: [q] };
      pages.push(current);
    }
  }
  return pages;
...
function validatePage(form: HTMLFormElement, pageIdx: number): string | null {
...
export default function SurveyForm() {
  ...
  const goNext = () => {
    ...
    if (!form) return;
    const err = validatePage(form, pageIndex);
    if (err) {
      showErrorToast(err);
      return;
    }
